...
export async function seedBetsData() {
  try {
    ...
    if (betsError) {
      ...
      return {
        success: false,
        message: 'Error seeding bets data',
        error: betsError,
      };
    }
    ...
    if (bets && bets.length > 0) {
      ...
      console.log(
        `Inserting ${oddsHistoryData.length} odds history records...`
      );

      const { error: oddsHistoryError } = await supabase
        .from('odds_history')
        .insert(oddsHistoryData);
      ...
      console.log(
        `Successfully inserted ${oddsHistoryData.length} odds history records`
      );
    }
